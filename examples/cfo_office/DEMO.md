...
> **Thesis:** Charter is the **Authority layer** — a signed, queryable work-contract that lets a
> generative multi-agent society reach **enforceable consensus** on who-does-what, so its
> collaboration is predictable, auditable, and convergent. The "CFO Office" society is the
> *showcase*. We are not building an accounting app — we are proving that **a generative society
> can agree, in signed text, what each agent may do, and that agreement holds even when an agent
> is compromised** (consensus at write time; teeth at read time).
